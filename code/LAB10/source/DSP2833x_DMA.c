...
void DMACH1AddrConfig( volatile Uint16* DMA_Dest, volatile Uint16* DMA_Source )  //配置DMA
{
	EALLOW;
	// Set up SOURCE address:        配置通道1的源地址
	DmaRegs.CH1.SRC_BEG_ADDR_SHADOW = ( Uint32 )DMA_Source;	// Point to beginning of source buffer    //映射
	DmaRegs.CH1.SRC_ADDR_SHADOW = ( Uint32 )DMA_Source;
	// Set up DESTINATION address:   配置通道1的目的地址
